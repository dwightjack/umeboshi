...
module.exports = (e = {}) => {
    ...
    const addAnalyzer = env.analyze
        ? (cfg) => {
              cfg.plugin('bundle-analyzer').use(BundleAnalyzerPlugin, [
                  {
    ...
    if (Array.isArray(packageConfig)) {
        return packageConfig.reduce(
            (a, c) =>
                a.concat(
                    toWebpackConfig(
                        mergeConfig(addAnalyzer(c), umeWebpack, env, true)
                    )
                ),
            []
        );
    }
    return toWebpackConfig(
        mergeConfig(addAnalyzer(packageConfig), umeWebpack, env)
    );
};
