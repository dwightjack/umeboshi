...
const base = require('umeboshi-config/modernizr/prod');

const toFile = (path, data) => new Vinyl({ path, contents: Buffer.from(data, 'utf8') });

const commons = glob.sync([
...
//test vue files...
const vue = glob.sync([
    paths.toPath('src.assets/js') + '/**/*.vue'
]).reduce((acc, file) => {
